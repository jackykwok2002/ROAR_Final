...
class SimpleWaypointFollowingLocalPlanner(LocalPlanner):
    ...
    def run_in_series(self) -> VehicleControl:
        ...
        while True:
            ...
            # START
            if self.brake_count > 40 and waypoint.location.x == 389.9820556640625 and waypoint.location.y ==  -0.009884776547551155 and waypoint.location.z == -89.15290832519531:
                    self.way_points_queue.popleft()

            if self.should_brake(waypoint.location): #added
                self.brake_count += 1
                control: VehicleControl = self.controller.run_in_series(next_waypoint=self.way_points_queue[self.brake_count])
                return VehicleControl(
    ...
    def should_brake(self, cur_target):
        print(cur_target.x, cur_target.y, cur_target.z)
        check = [(389.9820556640625,-0.009884776547551155,-89.15290832519531)]

        for element in check:
            if cur_target.x == element[0] and cur_target.y == element[1] and cur_target.z == element[2]:
                return True
        return False
